...
router.post("/", authenticate, async (req, res, next) => {
  try {
    ...
    await logActivity(req.user.id, "CREATE_ORDER", "Order", order.id, {
      orderNumber,
      totalAmount,
    });
...
router.put("/:id/status", authenticate, async (req, res, next) => {
  try {
    ...
    await logActivity(req.user.id, "UPDATE_ORDER_STATUS", "Order", order.id, {
      status,
    });
...
router.put("/:id/payment", authenticate, async (req, res, next) => {
  try {
    ...
    await logActivity(req.user.id, "UPDATE_PAYMENT", "Order", order.id, {
      paymentStatus,
    });
