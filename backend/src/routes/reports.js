...
const calculateStats = (orders) => {
  ...
  const paidOrders = orders.filter(
    (o) => o.paymentStatus === "SUDAH_BAYAR",
  ).length;
  const unpaidOrders = orders.filter(
    (o) => o.paymentStatus === "BELUM_BAYAR",
  ).length;
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=laporan-${type}-${Date.now()}.xlsx`,
      );
      res.send(buffer);
    } catch (error) {
      next(error);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      doc
        .fontSize(20)
        .text(settings?.businessName || "Laundry", { align: "center" });
      ...
      orders.slice(0, 50).forEach((order, i) => {
        doc.text(
          `${i + 1}. ${order.orderNumber} | ${order.customer.name} | Rp ${parseFloat(order.totalAmount).toLocaleString("id-ID")} | ${order.status}`,
        );
      });
