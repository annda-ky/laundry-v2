...
// Prevent double /api/api if user accidentally adds /api to the env var
...
class ApiClient {
  ...
  toggleUserActive(id) {
    return this.patch(`/api/users/${id}/toggle-active`);
  }
  deleteUser(id) {
  ...
  updateService(id, data) {
    return this.put(`/api/services/${id}`, data);
  }
  deleteService(id) {
  ...
  updatePayment(id, data) {
    return this.put(`/api/orders/${id}/payment`, data);
  }
  getReceipt(id) {
    return this.get(`/api/orders/${id}/receipt`);
  }

  // Dashboard
  getOwnerDashboard() {
    return this.get("/api/dashboard/owner");
  }
  getKasirDashboard() {
    return this.get("/api/dashboard/kasir");
  }
  ...
  exportExcel(type, startDate, endDate) {
    ...
    window.open(
      `${this.baseUrl}/api/reports/export/excel?${params}&token=${token}`,
      "_blank",
    );
  }
  exportPdf(type, startDate, endDate) {
    ...
    if (endDate) params.append("endDate", endDate);
    window.open(
      `${this.baseUrl}/api/reports/export/pdf?${params}&token=${token}`,
      "_blank",
    );
  }
